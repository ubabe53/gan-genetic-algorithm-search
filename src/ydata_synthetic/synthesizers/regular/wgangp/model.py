...
class WGAN_GP(gan.Model):
    ...
    def gradient_penalty(self, real, fake): # Passed the real data and the fake data
        epsilon = tf.random.uniform([real.shape[0], 1], 0.0, 1.0, dtype=tf.dtypes.float32)
        x_hat = epsilon * real + (1 - epsilon) * fake  # Sampling uniformly along straight lines between pairs of points sampled from the data distribution Pr and Pg
        with tf.GradientTape() as t:
            t.watch(x_hat) # It has to watch the variable. If not wouldn't be able to calculate the gradients
            d_hat = self.critic(x_hat)
        gradients = t.gradient(d_hat, x_hat) # Calculating the gradients
        ddx = tf.sqrt(tf.reduce_sum(gradients ** 2)) # Calculating the norm2 of gradients
        d_regularizer = tf.reduce_mean((ddx - 1.0) ** 2) # Gradient penalty for discriminator
    ...
    def get_data_batch(self, train, batch_size, seed=2):
        # np.random.seed(seed)
